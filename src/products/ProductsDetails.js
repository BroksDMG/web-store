...
import {FaShoppingBasket, FaHeart,FaCaretRight,FaCaretLeft } from "react-icons/fa";
...
function ProductsDetails() {
    ...
    const onClickLeftHandler=()=>{
      if(inputValue>=2){setInputValue(inputValue-1);}
    }
  ...
  return (
    <div className="max-w-[1440px]  h-full flex flex-col">
      ...
      <div className=" font-robo font-bold tracking-wider p-3 relative flex-grow flex flex-col justify-between bg-gray-100 h-full">
        ...
        <div className="flex w-full justify-end items-center z-10">
          ...
          <button onClick={onClickLeftHandler} className="bg-gray-200 w-full h-full rounded-l-lg   focus:outline-none active:outline-none">
          <FaCaretLeft className="text-gray-400"/>
          </button>
          <input type="Number"min={1} max={99} pattern="[1-9]{1,2}" value={inputValue} onChange={onChangeHandler} className="h-8  w-12 bg-transparent border-none text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
          <button onClick={onClickRightHandler} className="bg-gray-200 w-full h-full rounded-r-lg   focus:outline-none active:outline-none">
        ...
        <div className="my-2 mb-4">
            <ul className="flex flex-gap gap-2 text-sm overflow-hidden">
                <li onClick={()=>onClickBtnsHandler("opis")} className="min-w-max flex justify-center rounded-2xl border-2 p-1 border-gray-200 hover:bg-gray-200  ">Opis</li>
                <li onClick={()=>onClickBtnsHandler("sposob")} className="min-w-max flex justify-center rounded-2xl border-2 p-1 border-gray-200 hover:bg-gray-200  ">Sposób użycia</li>
                <li onClick={()=>onClickBtnsHandler("zastosowanie")} className="min-w-max flex justify-center rounded-2xl border-2 p-1 border-gray-200 hover:bg-gray-200  ">Zastosowanie</li>
            </ul>
        </div>
